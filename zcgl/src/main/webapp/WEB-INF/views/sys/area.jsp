...
<head>
    ...
    <script type="text/javascript">
        ...
        function loadTree() {
            me.tree1.tree({
                url: me.actionUrl + 'getAreaTree.do',
                onClick: function (node) {
                    me.edit_form.find('#areaParentcode').val(node.id);
                    me.edit_form.find('#parentName').textbox().textbox('setValue', node.text);
                   me.edit_form.find('#areaGrade').textbox().textbox('setValue', node.attributes);
                    me.edit_form.find('#state').attr('checked', true);
                    me.datagrid1.datagrid('reload');
                },
                onLoadSuccess: function (node, param) {
                    var selectedNode = $(this).tree('find', me.edit_form.find('#areaParentcode').val());
                    me.edit_form.find('#areaGrade').textbox().textbox('setValue', selectedNode.attributes);
                    if (selectedNode) {
        ...
        function loadGrid() {
            me.datagrid1.datagrid({
                idField: me.idFiled,
                url: me.actionUrl + 'getAreaList.do',
                frozenColumns: [[
                    {
                        field: 'state',
                        title: '状态',
                ...
                columns: [[
                    ...
                    {field: 'areaName', title: '名称', width: 200, align: 'center'},
                ...
                onBeforeLoad: function (param) {
                    param.areaParentcode = me.edit_form.find('#areaParentcode').val();
        ...
        function AddOrUpdate(action) {
            switch (action) {
                case 'add':
                    me.edit_form.find('#state').attr('checked', true);
                    me.edit_form.find('#id,#areaCode,#areaName,#backup1').textbox().textbox('setValue', '');
        ...
        <div region="center" border="false" style="padding: 5px; background: #fff; border: 1px solid #ccc;">
            <form id="edit_form" name="edit_form" method="post">
                <div title="隐藏参数" style="display: none">
                    ...
                    <input type="hidden" id="areaGrade" name="areaGrade"/>
                </div>
                <table cellpadding="3" align="center">
                    <tr>
                        <td>编码：</td>
                        <td>
                            <input id="areaCode" name="areaCode" type="text" class="easyui-textbox"
                                   style="width: 250px;"
                                   required="true"/>
                        </td>
                    </tr>
                    <tr>
                        <td>名称：</td>
                        <td>
                            <input id="areaName" name="areaName" type="text" class="easyui-textbox"
                                   style="width: 250px;"
                                   required="true"/>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input id="parentName" name="parentName" class="easyui-textbox" style="width: 250px;"
                                   readonly="true"
                                   value="河南省"/>
                            <input id="areaParentcode" name="areaParentcode" type="hidden"
                                   style="width: 50px; display: none;"
                                   reuired="true" value="410000"/>
                        </td>
                    </tr>
                    <tr>
                        <td>排序：</td>
            ...
            <form id="search_form" method="post">
                <table>
                    <tr>
                        ...
                        <td>
                            <input name="areaName" id="areaName" class="easyui-textbox" style="width: 200px;"/>
                        </td>
                    </tr>
                </table>
            </form>
