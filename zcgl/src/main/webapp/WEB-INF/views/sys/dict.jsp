    ...
    <script type="text/javascript">
        ...
        function loadGrid() {
            me.datagrid1.datagrid({
                idField: me.idFiled,
                url: me.actionUrl + 'getDictList.do',
                frozenColumns: [[
                    {
                        field: 'state',
                        title: '状态',
                        ...
                        formatter: function (value, row, index) {
                            var strReturn = '';
                            if (value == '1') {
                                strReturn = '<a href="javascript:void(0)" class="l-btn-text icon-ok" title="点击改变状态" onClick="UpdateState(\'' + row.dictId + '\',\'' + value + '\')"  style="padding-left:20px;height:20px;">&nbsp;&nbsp;</a>';
                            } else {
                                strReturn = '<a href="javascript:void(0)" class="l-btn-text icon-no" title="点击改变状态" onClick="UpdateState(\'' + row.dictId + '\',\'' + value + '\')" style="padding-left:20px;height:20px;">&nbsp;&nbsp;</a>';
                            }
                            return strReturn;
                        }
                    }
                ...
                columns: [[
                    {field: 'dictCode', title: '编码', width: 100, align: 'center'},
                    {field: 'dictName', title: '名称', width: 200, align: 'center'},
        ...
        function AddOrUpdate(action) {
            switch (action) {
                case 'add':
                    me.edit_form.find('#state').attr('checked', true);
                    me.edit_form.find('#dictId,#dictCode,#dictName,#backup1').textbox().textbox('setValue', '');
                    me.edit_window.find('div[region="south"]').css('display', 'block');

                    if (me.edit_form.find('#parentName').val() == "系统基础代码")
                        me.edit_form.find('#groupName').textbox().textbox('setValue', '');

                    if (me.edit_form.find('#groupName').val() == "POSITIONAL_TITLES")
                        me.edit_window.find('#backup2TR').css('display', '');
                    else
                        me.edit_window.find('#backup2TR').css('display', 'none');
                ...
                case 'update':
                    ...
                    if (selectedRows.length > 0) {
                        $.ajax({
                            url: me.actionUrl + 'AddOrUpdate.do?dictId=' + selectedRows[0][me.idFiled],
        ...
        function UpdateState(dictId, state) {
            $.ajax({
                url: me.actionUrl + 'updateState',
                data: {dictId: dictId, state: state},
                success: function (returnData) {
                    if (returnData) {
                        if (returnData.isOk == 1) {
                            me.datagrid1.datagrid('reload');
...
<div region="west" hide="true" split="true" title="导航树" style="width: 380px;">
    <ul id="tree1">
    </ul>
</div>
...
<div id="edit_window" class="easyui-window" closed="true" title="数据维护窗口"
     style="width: 400px;height: 410px; padding: 5px;">
            ...
            <form id="edit_form" name="edit_form" method="post">
                ...
                <table cellpadding="3" align="center">
                    <tr>
                        <td>编码：</td>
                        <td>
                            <input id="dictCode" name="dictCode" type="text" class="easyui-textbox"
                                   style="width: 250px;"
                                   required="true"/>
                        </td>
                    </tr>
                    <tr>
                        <td>名称：</td>
                        <td>
                            <input id="dictName" name="dictName" type="text" class="easyui-textbox"
                                   style="width: 250px;"
                                   required="true"/>
                        </td>
                    </tr>
                    <tr>
                        <td>分组名：</td>
                        <td>
                            <input id="groupName" name="groupName" type="text" class="easyui-textbox"
                                   style="width: 250px;"
                                   required="true"/>
                        </td>
                    </tr>
                    <tr id="backup2TR" style="display: none">
                        <td>
                            职称级别：
                        </td>
                        <td>
                            <input id="backup2" class="easyui-combobox" name="backup2" value="" style="width: 250px"
                                   maxlength="30"
            ...
            <form id="search_form" method="post">
                <table>
                    <tr>
                        ...
                        <td>
                            <input name="dictName" id="dictName" class="easyui-textbox" style="width: 200px;"/>
                        </td>
                    </tr>
                </table>
            </form>
