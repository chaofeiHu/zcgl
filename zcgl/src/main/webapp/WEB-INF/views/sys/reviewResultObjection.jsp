...
<head>
    ...
    <script type="text/javascript">
        ...
        var me = {
            ...
            actionUrl: '<%=basePath%>/ResultObjection/'
        };
        ...
        function loadGrid() {
            ...
            me.datagrid1.datagrid({
                ...
                columns: [[
                    ...
                    {field: 'reviewResult', title: '大评会是否通过', width: 130, align: 'center',
                        formatter: function (value, rowData, rowIndex) {
                            if(value==1||value=="1") return "通过";
                            if(value==0||value=="0") return "不通过";
                    ...
                    {field: 'opt', title: '操作', width: 340, align: 'center',
                        formatter: function (value, rowData, rowIndex) {
                            var strReturn = '';
                            strReturn += '<a href="javascript:void(0)"   title="转评审通过人员库" onClick="toResultManage(\'' + rowData['id'] +'\');" style="padding-left:20px;" >转评审通过人员库</a>'
                            strReturn += '<a href="javascript:void(0)"   title="转评审未通过人员库" onClick="toDissent(\'' + rowData['id'] +'\');" style="padding-left:20px;" >转评审未通过人员库</a>'
                            return strReturn;
                        }
                    },
                ...
                toolbar: [
                    {
                        text: '转评审通过人员库', iconCls: 'icon-ok', handler: function () {
                    ...
                    {
                        text: '转评审未通过人员库', iconCls: 'icon-redo', handler: function () {
                        var selectedRows = me.datagrid1.datagrid('getSelections');
                        var ids = [];
                        if (selectedRows.length == 0) {
                            showError('请选择一条记录进行操作!');
                    ...
                    {
                        text: '导出异议人员结果', iconCls: 'icon-excel', handler: function () {
        ...
        function toDissent(ids) {
            $.messager.confirm('提示信息', '是否确认转入未通过人员库？', function (isClickedOk) {
                if (isClickedOk) {
                    $.ajax({
